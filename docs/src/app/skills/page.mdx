...
This fork adds `camoufox`: use `agent-browser skills get camoufox` for the isolated runtime, native AI snapshot/capture workflow, motion profiles, schema-discovered gestures, and explicit V1 safety limits. Its MCP entry point is `agent_browser_skills_get` in the `gestures` profile. [Camoufox setup](/engines/camoufox) describes the built-binary prerequisite and the `core,gestures` profile combination.
