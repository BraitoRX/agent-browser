...
For this fork's Camoufox backend, use the separate [Camoufox setup guide](/engines/camoufox). The upstream npm, Homebrew, and Cargo distributions below do not contain the fork backend. The initial source requires a build and authorized native/browser acceptance; adding source files does not activate a running service.
