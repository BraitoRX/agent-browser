...
agent-browser page-outline [sel]      # Structured headings and landmarks (Camoufox)
agent-browser page-links [sel]        # Paginated actionable links (Camoufox)
agent-browser dom-chunk [sel]         # Paginated DOM records with @dN refs (Camoufox)
...
## Structured page inspection (Camoufox)

```bash
agent-browser page-outline [selector]
agent-browser page-links [selector] [--cursor <cursor>] [--limit <1-200>]
agent-browser dom-chunk [selector] [--cursor <cursor>] [--limit <1-500>]
agent-browser get html "html"
agent-browser click "xpath=//button[@type='submit']"
```

Use `page-outline` as a bounded DOM-derived map of headings, landmark-like regions, and element counts. `page-links` takes one native accessibility snapshot and returns actionable refs, accessible text, resolved URLs, and nearest heading context. `dom-chunk` returns structured element records in DOM preorder with document-scoped actionable `@dN` refs. Each command operates only in the selected frame, and an explicit root must match exactly one element there. Select an iframe before inspecting its links or DOM. For both paginated commands, pass a selector only on the first call and then follow the opaque `nextCursor`; omit `limit` on continuations or keep it unchanged. A changed document makes a DOM cursor stale; navigation, frame changes, snapshots, and mutating actions invalidate `@dN` refs. The DOM inventory exposes at most 50000 addressable elements and reports `omittedNodes` when the document is larger.

Selectors can be observation refs, direct CSS, or XPath prefixed with `xpath=`. `get html "html"` returns the whole document element's inner HTML, but raw strings remain subject to the backend output ceiling. Prefer structured pagination over a large HTML dump.
