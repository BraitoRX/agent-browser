...
This fork accepts `engine: "camoufox"` or `--engine camoufox` for its explicit V1 subset. Camoufox uses separate install-time/runtime environment variables and rejects unsupported Chrome configuration such as profiles, restore, proxies, CDP, and domain containment. See [Camoufox configuration and MCP](/engines/camoufox) before reusing an existing config file.
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>adblock</code></td><td><code>--adblock</code> or <code>AGENT_BROWSER_ADBLOCK</code></td><td>boolean; Camoufox only, off by default</td></tr>
