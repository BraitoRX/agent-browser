...
- Timeout policy: 41 fake-based tests plus Rust `camoufox_timeout` tests separate cleanly released operation timeouts (`data.timeoutKind: "operation"`, session preserved) from worker deadlines (`"deadline"`, poisoned only when input was attempted during the action, journaled input could not be released, or a launch/close was cancelled; a cancelled read-only action stays unpoisoned). Failed releases, non-timeout ambiguous input, and transport failures keep the no-replay safety boundary.
