...
- Verified element identity and inside-out discovery: `element-inspect` (MCP `agent_browser_element_inspect`) returns an `element_card` for one element with a per-document `elementId` that stays stable for the node's lifetime and is never reassigned, plus CSS `candidates` that are marked usable only when each resolves to exactly one element and that element is the same node. `element-expand` (MCP `agent_browser_element_expand`) walks `parent`, `ancestors`, `siblings`, or `subtree` from that id and returns lightweight records with component-boundary flags instead of HTML, so an agent can decide whether to ascend without reading the page. Both are read-only: no input is dispatched, and captures and refs survive.
- Visual point resolution: `visual-target` (MCP `agent_browser_visual_target`) turns a screenshot pixel into a verified element target. It converts image pixels to CSS, revalidates the capture, hit-tests the stack, and returns an interior `snappedPoint` at the element box centre, so callers use the backend-verified point rather than a raw model estimate. Read-only; `expectElementId` fails with `camoufox_stale_ref` when the expected node is no longer at that point.
- `find` now verifies the selector it returns. Candidates are ranked stable-key first, then a document-unique `id`, then a keyed ancestor with relative segments; an action proceeds only when the winner resolves to exactly one element and that element is the matched node. A non-unique or unresolvable candidate fails with `camoufox_invalid_params` instead of delegating input to a guessed target.
...
The container is named after the session: a named session `task1` produces the container `agent-browser-bubble-task1`, and an anonymous default session keeps the ephemeral PID+nanosecond name. On OrbStack every container also gets a fixed domain derived from its name, so a named session has a stable noVNC address across relaunches: `https://agent-browser-bubble-task1.orb.local/vnc.html?autoconnect=1&quality=6&compression=0&resize=scale&reconnect=1&view_only=1`. Launch and `session info` report it as `vncDomainUrl` alongside the port-based `vncUrl`. The viewer settings travel in the query string so they always apply regardless of the viewer browser's storage: `quality=6` keeps JPEG encoding cheap on CPU (the frames are software-rendered), `compression=0` skips zlib (localhost bandwidth is not a constraint), `resize=scale` scales on the client side for free, `reconnect=1` reattaches automatically when the container relaunches, and `view_only=1` opens the viewer in watch mode so observing does not race the agent for the X cursor; uncheck View Only in the viewer panel when manual interaction (login, CAPTCHA) is needed. Use a distinct `--session` per concurrent bubble to keep each domain unique; a leftover container with the same name from an unclean previous shutdown is removed automatically before the new one starts.
...
The `gestures` profile adds discovery and execution, installation, session information, and skill retrieval. When started with Camoufox, MCP advertises only supported tools and arguments, including in `all`, and rejects engine overrides; generic Chrome servers keep their existing catalog. Core includes `agent_browser_find` (semantic locators `role`/`text`/`label`/`placeholder`/`alt`/`title`/`testid`/`first`/`last`/`nth` with an optional `click`/`fill`/`check`/`hover`/`text` action), `agent_browser_element_inspect` and `agent_browser_element_expand` (verified element identity, candidate selectors, and inside-out expansion without reading page HTML), `agent_browser_visual_target` (resolve a screenshot pixel into a verified element with a snapped interior point), `agent_browser_hover_hold` (keep native mouse micro-movement on one selector so hover-revealed UI stays visible while you observe; screenshots and reads continue during the hold, and it auto-stops before any input or lifecycle action), full-page HTML reads via `agent_browser_get_html`, HTML/attribute/value/count/bounds/state queries, and `scroll_into_view`. The `page-outline`, `page-links`, and `dom-chunk` commands are not exposed as MCP tools on any profile; they remain CLI commands. Normal calls default to 120 seconds and require `timeoutMs >= 30000`; explicit installation has a longer default. Retrieve `agent_browser_skills_get` with `names: ["camoufox"]`. Native AI snapshots already include URLs and cursor markers, so no extra MCP flags are needed. Interactive/compact filtering is unavailable; CLI `--urls`/`--cursor` are compatibility requests for always-present native annotations.

```bash
"$AB" page-outline --json
"$AB" page-links --limit 50 --json
"$AB" dom-chunk --limit 100 --json
"$AB" element-inspect --selector "#save" --geometry --json
"$AB" element-expand el_a1b2c3d4_42 --relation ancestors --limit 5 --json
"$AB" visual-target "$CAPTURE_ID" 812 340 --json
"$AB" get html @d12
"$AB" click "xpath=//button[@type='submit']"
"$AB" get html "html"
```

Snapshots expose an accessibility view, not the whole DOM. Keep their `@eN` refs as the default interaction mechanism. A main-frame ref whose element was re-rendered is re-resolved by role/name automatically; an ambiguous or empty re-resolution returns `camoufox_stale_ref` and needs a fresh snapshot, and iframe-prefixed refs such as `@f1e2` have no fallback. `find` locates elements semantically (`role`, `text`, `label`, `placeholder`, `alt`, `title`, `testid`, `first`, `last`, `nth`) and its read-only `text` subaction returns the element's text plus a CSS selector reusable by any selector command. That selector is verified before it is returned: the worker prefers a stable key, then a document-unique `id`, then a keyed ancestor with relative segments, and accepts a winner only when it resolves to exactly one element and that element is the matched node; otherwise the action fails instead of returning a non-unique path. Acting subactions reject zero and ambiguous matches before any input. `element-inspect` returns one element's identity card without reading the page, and `element-expand` walks its `parent`, `ancestors`, `siblings`, or `subtree` with component-boundary flags and no HTML, which is the cheapest way to decide whether to ascend toward a parent component; `visual-target` converts a screenshot pixel into a verified element with a snapped interior point. `page-outline` returns a bounded DOM-derived summary of headings, landmark-like regions, and counts. `page-links` takes one native snapshot and pages through actionable link refs, resolved URLs, and nearest heading context. `dom-chunk` pages through structured preorder element records with document-scoped actionable `@dN` refs. Each pagination command operates only in the selected frame, and an explicit root must match exactly one element there. Select an iframe before inspecting its links or DOM. Pass only the returned opaque `nextCursor` on continuations, omitting `limit` or keeping it unchanged; a changed document makes a DOM cursor stale, while navigation, frame changes, snapshots, and mutating actions invalidate `@dN` refs. The three pagination commands are CLI/daemon commands, not MCP tools; MCP clients use `agent_browser_find`, `agent_browser_element_inspect`, `agent_browser_get_html` with selector `html`, and `agent_browser_snapshot` instead. Selectors accept direct CSS or XPath prefixed with `xpath=`. `get html "html"` returns the whole document element's inner HTML, but raw DOM or eval strings still face the 16 MiB response ceiling without pagination. CSS locators pierce open shadow roots, while JavaScript and structured DOM traversal do not automatically expand descendant shadow roots. Closed roots are not exposed. Element identity and selector candidates are resolved with native `querySelectorAll` semantics, which do not cross shadow boundaries: an element inside an open shadow root is reported with `inShadowRoot` and its structural candidates are flagged rather than silently crossing the boundary.
