...
fn launch_connection_is_external(cdp_url: Option<&str>, cdp_port: Option<u64>, auto_connect: bool) -> bool {
    launch_connection_identity(cdp_url, cdp_port, auto_connect).0 != "local"
}
...
fn has_active_browser_session(state: &DaemonState) -> bool {
    state.browser.is_some() || state.camoufox.is_some()
}
...
async fn handle_launch(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    ...
    let (connection_kind, connection_target) =
        launch_connection_identity(cdp_url, cdp_port, auto_connect);
