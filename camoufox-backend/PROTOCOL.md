...
| `camoufox_poisoned` | browser action refused after a worker deadline with attempted or unreleased input, unreleased input on another failure path, or another ambiguous failure |
...
MCP recovery hints use plain language. An unpoisoned timeout adds: "The browser is still available. Inspect the current page before continuing; do not automatically replay input." Agents should treat this as a failed step, observe the same session, and choose the next step from fresh evidence rather than restarting or abandoning the task merely because a wait expired. The structured safety fields and explicit-close requirements are unchanged.
...
- On worker deadline expiry: attempt a bounded release in `finally`, then poison the worker only when the cancelled action may have left native input, a half-started browser, or another resource in an indeterminate state: journaled buttons/keys still pending after the release attempt, native input attempted during the action, or a cancelled launch/close. A cancelled read-only action (a locator read, snapshot, wait, or other action that dispatched no input) keeps the session usable; it returns `camoufox_timeout` with `data.timeoutKind: "deadline"` and no `poisoned` flag. Unreleased input, a non-timeout exception after attempted input, and an operation timeout with failed release retain `poisoned:true`. Further browser actions are refused (`camoufox_poisoned`) only after poisoning; `close`, `session_info`, `tab_list`, and `gestures` always remain. No replay or restart. Pending unreleased input cannot produce a success response. Rust marks transport failures after sending as ambiguous and suppresses CLI retries/respawn replay.
...
`download` installs its event expectation before exactly one guarded click, then waits up to 10 seconds after that click completes. A normal no-event timeout is a nonpoisoning `camoufox_error`; inspect metadata or retry `waitfordownload`, never the triggering input. `waitfordownload` likewise reports an ordinary no-event timeout without poisoning and rechecks session liveness before advising a retry. The overall worker deadline remains authoritative: deadline expiry during attempted input still poisons the session and requires explicit close. This recovery discards retained download handles. `session_info.capabilities.interactionEvents` reports the active-tab scope, retention cap and event-wait limit.
