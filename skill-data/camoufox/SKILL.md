...
- `fast` and `precision` disable native humanization. Change `AGENT_BROWSER_MOTION` before starting a new daemon session, not during a gesture.
...
### Remote page failures

A rendered site error is application state, not a browser failure. Sites ship their own crash screens: "Something went wrong", "the app hit an error on our end", HTTP 500 pages, hydration failures in site-owned JavaScript chunks. None of these carry a `camoufox_` error code, the session stays usable, and navigation succeeded; the remote application failed after loading. Diagnose in class order before touching the session:

1. Classify the failure first: browser failure (`camoufox_*` code), navigation or transport failure (DNS error, HTTP status, connection refused), challenge or block page (CAPTCHA, Cloudflare interstitial; a blocker for the agent, never something to fix or evade), or remote application failure (the site's own error text rendered as page content).
2. Capture evidence once: one `read` or screenshot of the error screen, one `errors` and `console` inspection. A JavaScript error originating in the site's own chunks (for example `https://site.com/_next/static/...`) confirms a remote defect. A rejected-cookie warning alone does not.
3. Verify out-of-band only when it changes the decision: a host-side fetch of the same URL that also returns the error confirms a server-side defect. HTTP 200 with the error still rendered means the server is up and the site's own frontend is broken.
4. Continue or stop, but do not repair a remote defect with local actions. Use an alternative route the site itself offers (its public JSON API, a different page, a search engine cache) or report the blocker to the user with the captured evidence.

The retry budget for any single remote error is: at most one activation of the page's own retry control (such as "Try again"), at most one manual reload, then stop retrying that URL for the task. The same error persisting across two fresh observations is conclusive. Do not clear cookies or site storage, restart the session, switch profiles, toggle the VPN, or otherwise alter identity or state to fix a site's own defect: those actions require the user's authorization and are ineffective against remote bugs. Report and move on rather than looping. In user-facing explanations, say "the site's own application failed" rather than claiming navigation or the session is broken.

### Recovering page data from a broken frontend

When a site's frontend crashes but the server responded correctly, the original server response stays in the network capture: the page failed after receiving a complete HTTP response, and `network request <id>` can return that body even though the DOM only shows the error screen. This is the preferred recovery route before any workaround, and it needs one preparatory habit:

- On a fresh site (or before a risky page), start a capture with `network requests` first or note the document request right after navigation: open the page, then immediately call `network requests` with `type: "document"` and the site's domain as filter. Note the returned `requestId` of the main document. Document records survive sub-request eviction, so this id stays usable on heavy SPA pages.
- When the frontend then crashes, call `network request <requestId>` on that document id. The response includes `responseBody` with the raw server HTML (up to 8 MiB for documents; larger bodies report `responseBodyUnavailable` with a reason and `responseHeaders` stay available). Parse the embedded data from there: Next.js pages carry the full payload in inline `self.__next_f.push` scripts; other frameworks embed JSON islands, `__NEXT_DATA__`, or similar. The error boundary never replaced what the server actually sent.
- Read the body from inside the owning session; never copy the page through external container tools as a workaround. If the body is missing, the reported `responseBodyUnavailable.reason` says why (`oversized`, `pending`, `failed`, `unavailable`): a `failed` document means the server genuinely did not deliver, while any rendered error screen with a successful document response is a frontend-only failure.
- This recovery works within the live session. Session close, reset, or a reload cycle clears retained captures; capture the body before any recovery step that reopens the page.
