...
- Keep snapshot `@eN` refs as the default interaction mechanism. Native snapshots cover the accessibility view, not every DOM node. Scope with an observed selector or ref and optionally `depth` (0–100; zero means unlimited). Selectors may be CSS or XPath prefixed with `xpath=`. URLs are also available as raw `refs[ref].url`, not automatically resolved absolute URLs. Navigation/frame detachment clears refs; DOM-only re-rendering can still produce native locator errors or timeouts. Reobserve instead of assuming every stale node gets the same error code.
- On a large page, call `page-outline` / `agent_browser_page_outline` first for a bounded DOM-derived summary of headings, landmark-like regions, and counts. Use `page-links [selector] --limit N` / `agent_browser_page_links` when you need a structured link inventory. Its first page takes one native snapshot and returns actionable `@eN` refs, resolved URLs, heading context, and an opaque `nextCursor`; pass only that cursor on continuation pages, omitting `limit` or keeping it unchanged, so the inventory and refs remain stable.
- Use `dom-chunk [selector] --limit N` / `agent_browser_dom_chunk` for low-level elements absent from accessibility output. It returns structured preorder records with document-scoped actionable `@dN` refs. Page inspection operates only in the selected frame, and an explicit root must match exactly one element there; select an iframe first when needed. Pass only `nextCursor` on continuations, omitting `limit` or keeping it unchanged. A changed document makes the cursor stale; navigation, frame changes, snapshots, and mutating actions invalidate `@dN` refs. Start a fresh chunk rather than inventing or reusing stale refs.
...
- For offscreen targets, use native locator actions or `scroll_into_view` on an observed unique target instead of guessing long scroll distances. Native action dispatch is not proof of application success. Screenshots are viewport PNG only; do not request JPEG, annotation or full-page capture.
- Raw DOM strings and eval results still have a 16 MiB response ceiling and no automatic pagination. Prefer `dom-chunk` for structured pagination; otherwise narrow selectors or explicitly slice/map a small result in `eval`. An oversized-output error is not evidence that the DOM is empty. Semantic `find`, arbitrary selector-engine chaining and closed-shadow inspection are not advertised.
...
Download waits consume only the active tab's events; switch explicitly to a live source tab when needed. Saving requires filesystem hard-link support for atomic publication and produces mode-0600 files. A poisoned session cannot retry saving: follow the explicit-close recovery rule instead.
...
- A completed Playwright operation timeout with `data.timeoutKind: "operation"` and no `poisoned: true` does not require closing the session. Take a fresh observation in the existing session and inspect whether input took effect before deciding what to do next. Do not automatically replay the failed click, typing, submission, or gesture. If poisoning is reported, explicit-close recovery takes precedence. If the same failure recurs after a deliberate correction, stop and report the diagnostics instead of looping or repeatedly resetting the browser.
...
- Distinguish operation timeouts from worker failures. A completed Playwright `TimeoutError` returns `camoufox_timeout` with `data.timeoutKind: "operation"` and remains unpoisoned when input cleanup succeeds. Keep the session and inspect the page; **do not automatically replay input**. Actual worker deadline expiry returns `data.timeoutKind: "deadline"` and still poisons the session. Unreleased input, non-timeout failures after attempted input, and ambiguous transport errors also retain explicit-close recovery. Release attempts and dispatch counters are not evidence that the application operation did or did not happen.
- Playwright errors include the action, exception class, and a bounded browser reason with literal submitted input/code redacted. MCP text includes `camoufox_` codes and poisoned-session warnings. Use the actual reason, not the class name alone: multiple matches require a unique observed target; a missing snapshot scope calls for a fresh unscoped observation, not guessed selectors or unrelated navigation. Only correct targets in a usable, unpoisoned session when input was not possibly dispatched. See the [error-detail reference](../core/references/camoufox.md#browser-error-details).
