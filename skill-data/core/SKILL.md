...
Camoufox also supports `--input-backend os-native`: input dispatch through XTEST against the session's private X display inside a container bubble, with a live noVNC view (`vncUrl`) in the launch response. Sessions, cookies, and storage persist exactly as with the default `juggler` backend. It requires Docker/OrbStack and a one-time image build (`camoufox-backend/bubble/build.sh`); see [the Camoufox reference](references/camoufox.md#input-backends) for the honest scope: window-system-level input, not a host HID device, and no indetectability claim.
