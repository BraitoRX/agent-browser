...
A Camoufox-started MCP server exposes an engine-specific catalog, not the full Chrome surface. Its native snapshots already include URLs/cursor markers and replace exposed refs on every scoped or unscoped snapshot. Core includes `agent_browser_find` with semantic locators (`role`, `text`, `label`, `placeholder`, `alt`, `title`, `testid`, `first`, `last`, `nth`), `agent_browser_element_inspect` and `agent_browser_element_expand` for verified element identity and inside-out discovery without reading page HTML, `agent_browser_visual_target` for resolving a screenshot pixel into a verified element point, full-page HTML reads via `get html "html"` / `agent_browser_get_html`, `agent_browser_hover_hold` for keeping hover-revealed UI visible while observing it, HTML/attribute/value/count/bounds/state queries, and native reveal. The `page-outline`, `page-links`, and `dom-chunk` commands remain CLI-only for Camoufox and are not exposed as MCP tools on any profile. Use the `tabs` profile for explicit frame scope from observed frame IDs; see the specialized skill for nested frames, open shadow roots and output limits. `V1` is the private protocol/runtime namespace, not an old tool version to replace with OpenCode V2.
...
Camoufox also supports `--input-backend os-native`: input dispatch through XTEST against the session's private X display inside a container bubble, with a live noVNC view (`vncUrl`) in the launch response. The container is named after the session, so on OrbStack the live view also has a fixed domain (`vncDomainUrl`: `https://agent-browser-bubble-<session>.orb.local/vnc.html`). Sessions, cookies, and storage persist exactly as with the default `juggler` backend. It requires Docker/OrbStack and a one-time image build (`camoufox-backend/bubble/build.sh`); see [the Camoufox reference](references/camoufox.md#input-backends) for the honest scope: window-system-level input, not a host HID device, and no indetectability claim.

Camoufox can load the managed runtime's bundled uBlock Origin addon with `--adblock`, `AGENT_BROWSER_ADBLOCK`, config `adblock`, or the MCP `adblock` argument; it is off by default. It can have false positives that hide legitimate site content, so it is opt-in per session. Inspect `session info` for `adblock`; close the session before changing the setting. If a page reports missing content, close and reopen without adblock before assuming a page defect.
...
Camoufox Playwright failures include a bounded browser reason, not just the exception class. Use the reported reason to distinguish selector failures; do not guess that every `Error` means a missing element. A completed Playwright timeout reports `camoufox_timeout` with `data.timeoutKind: "operation"`. When `inputAmbiguous` is not true, do not close the session or abandon the task merely because of that timeout: inspect the current page in the same session and choose the next step from fresh evidence. Do not automatically replay input. A worker deadline (`data.timeoutKind: "deadline"`) requires close only when input was attempted during the action or journaled input could not be released; a cancelled read-only action keeps the session usable. MCP text uses plain-language recovery guidance; avoid internal jargon in user-facing explanations. Follow the [error-detail guidance](references/camoufox.md#browser-error-details), with no-replay safety taking precedence over selector correction.
...
Refs (`@e1`, `@e2`, ...) are assigned fresh on every snapshot. They become **stale the moment the page changes** — after clicks that navigate, form submits, dynamic re-renders, dialog opens. Main-frame refs whose element was re-rendered are re-resolved by role/name automatically; when that re-resolution is ambiguous or finds nothing, the action returns `camoufox_stale_ref` and you must take a fresh snapshot. Refs inside an iframe (`@fNeN`) have no such fallback. Always re-snapshot before your next ref interaction.
...
For large rendered pages on Camoufox, read the whole document element's inner HTML with `get html "html"` and locate elements semantically with `find`, for example `find role button text --name "Submit"`; the read-only `text` subaction prints the element's text and a CSS selector reusable by any selector-based command, and that selector is now verified to resolve to exactly one element that is the matched node, so a non-unique candidate fails instead of delegating input to a guess. Acting subactions reject zero and ambiguous matches before any input. Locate one element's identity and its component boundary with `element-inspect`, then ascend with `element-expand --relation ancestors` or `parent` instead of reading the page; both return bounded records with no HTML. Turn a screenshot pixel into a verified element point with `visual-target`. The CLI `page-outline`, `page-links --limit 50`, and `dom-chunk --limit 100` commands remain available for cursor-paginated headings, actionable link refs, and low-level element records with `@dN` refs; follow the returned `nextCursor` without repeating the selector. These pagination commands are not exposed as MCP tools on any profile.
...
Stable `tabId`s mean `t2` points at the same tab across commands even when other tabs open or close. After switching, refs from a prior snapshot on a different tab no longer apply — re-snapshot. `tab list --json` also reports each tab's CDP `targetId`, accepted anywhere a tab ref is accepted; target ids stay stable across daemon restarts, unlike `t<N>` ids. On the Camoufox engine, `tab new` opens the new tab inside the active browser window when a live active page exists; with no usable active page it falls back to a separate Camoufox window.
...
**Site shows its own error ("Something went wrong", "the app hit an error", HTTP 5xx)** The site's application failed after loading; navigation and the session are fine. This is not a browser failure, so browser-side remedies (reloading in a loop, clearing cookies, restarting the session, switching profiles) do not apply and can waste the task budget. Diagnose once: read or screenshot the error screen, and check `errors`/`console` — a JavaScript error originating in the site's own bundles (for example `https://site.com/_next/static/...`) confirms a remote defect, as does an out-of-band fetch of the same URL showing the same error. Retry budget for one remote error: at most one activation of the page's own retry control (like "Try again") plus one manual reload. If the error persists across two fresh observations, stop retrying that URL; continue through an alternative surface the site publishes (public JSON API, another page) or report the blocker to the user with the captured evidence. Never clear cookies, storage, or identity state to "fix" a site's own defect. A CAPTCHA or bot-check interstitial is a different case: it is a blocker for the agent, never something to solve or evade.

**Need the page content behind a broken frontend** The server response is still captured by the network observer even when the site's own JavaScript crashed. Find the document with `network requests` (`type: "document"` plus a domain filter), then `network request <id>` returns `responseBody` with the raw server HTML, including embedded framework data (Next.js `self.__next_f`, `__NEXT_DATA__`). Documents allow up to 8 MiB and survive sub-request eviction. Do this within the live session — closing or reloading clears the capture. (Camoufox backend: see the camoufox skill's "Recovering page data from a broken frontend".)
